...
class _AdminOrdersPageState extends State<AdminOrdersPage> {
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text("Supprimer", style: TextStyle(color: kCancelText)),
          ),
        ],
      ),
    );
  ...
  Color statusColor(String status) {
    switch (status) {
      case "payee":    return kBlue;
      case "expediee": return kGreen;
  ...
  Color statusBgColor(String status) {
    switch (status) {
      case "payee":    return kBlue.withOpacity(0.10);
      case "expediee": return kGreen.withOpacity(0.10);
      default:         return kOrange.withOpacity(0.12);
    }
  }

  IconData getPaymentIcon(String mode) {
    switch (mode) {
      case "cheque": return Icons.receipt_long;
      case "cb":     return Icons.credit_card;
  ...
  Widget _buildStatusBadge(String status) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: statusColor(status).withOpacity(0.25), width: 1.5),
      ),
      child: Text(
        formatStatus(status),
        style: TextStyle(color: statusColor(status), fontWeight: FontWeight.w700, fontSize: 11, letterSpacing: 0.5),
      ),
    );
  }
  ...
  Widget _buildPaymentRow(String mode) {
    return Row(
      children: [
        ...
        Text(getPaymentLabel(mode),
          style: TextStyle(fontSize: 12, color: Colors.grey[600], fontWeight: FontWeight.w500, letterSpacing: 0.4)),
      ],
    );
  }
        ...
        children: [
          Expanded(
            child: TextButton(
              ...
              style: TextButton.styleFrom(
                backgroundColor: kCancelBg, foregroundColor: kCancelText,
                padding: const EdgeInsets.symmetric(vertical: 13),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
              ),
              child: const Text("Supprimer", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
            ),
          ),
          ...
          Expanded(
            child: TextButton(
              ...
              child: const Text("Valider", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
            ),
          ),
        ],
      );
      ...
      children: [
        ...
        Expanded(
          child: TextButton(
            ...
            style: TextButton.styleFrom(
              backgroundColor: kBrown, foregroundColor: Colors.white,
              padding: const EdgeInsets.symmetric(vertical: 13),
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
            ),
            child: const Text("Expédier", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
          ),
        ),
      ],
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text(
          "Commandes",
          style: TextStyle(color: Color(0xFF1A1A1A), fontWeight: FontWeight.w700, fontSize: 28, letterSpacing: -0.5),
        ),
        actions: [
          IconButton(
                    ...
                    child: GestureDetector(
                      ...
                      child: AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(30),
                          border: Border.all(
                            color: isActive ? kBrown : const Color(0xFFDDDDDD),
                            width: 1.5,
                          ),
                        ),
